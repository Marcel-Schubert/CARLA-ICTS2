...
class AutoBotJoint(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(self.L_enc):
            tx_encoder_layer = nn.TransformerEncoderLayer(
                d_model=self.d_k, nhead=self.num_heads, dropout=self.dropout, dim_feedforward=self.tx_hidden_size
            )
        ...
        self.dec_agenttypes_encoder = nn.Sequential(
            init_(nn.Linear(2 * self.d_k, self.d_k)), nn.ReLU(), init_(nn.Linear(self.d_k, self.d_k))
        )
    ...
    def temporal_attn_fn(self, agents_emb, agent_masks, layer):
        ...
        agents_temp_emb = layer(
            self.pos_encoder(agents_emb.reshape(T_obs, B * (self._M + 1), -1)), src_key_padding_mask=agent_masks
        )
    ...
    def social_attn_decoder_fn(self, agents_emb, agent_masks, layer):
        ...
        B = agent_masks.size(0)
        agent_masks = (
            agent_masks[:, -1:].repeat(1, self.T, 1).view(-1, self._M + 1)
        )  # take last timestep of all agents.
    ...
    def forward(self, ego_in, agents_in, roads, agent_types):
        ...
        if self.use_map_lanes:
            ...
            map_features = (
                orig_map_features.unsqueeze(2).repeat(1, 1, self.c, 1, 1).view(-1, B * self.c * (self._M + 1), self.d_k)
            )
            road_segs_masks = (
                orig_road_segs_masks.unsqueeze(2).repeat(1, self.c, 1, 1).view(B * self.c * (self._M + 1), -1)
            )
        ...
        # embed agent types
        agent_types_features = (
            self.emb_agent_types(agent_types).unsqueeze(1).repeat(1, self.c, 1, 1).view(-1, self._M + 1, self.d_k)
        )
        ...
        for d in range(self.L_dec):
            if self.use_map_lanes and d == 1:
                ...
                agents_dec_emb_map = self.map_attn_layers(
                    query=agents_dec_emb, key=map_features, value=map_features, key_padding_mask=road_segs_masks
                )[0]
            ...
            agents_dec_emb = self.temporal_attn_decoder_fn(
                agents_dec_emb, context, opps_masks_modes, layer=self.temporal_attn_decoder_layers[d]
            )
        ...
        mode_params_emb = self.prob_decoder(
            ...
            key=agents_emb.reshape(-1, B * (self._M + 1), self.d_k),
            value=agents_emb.reshape(-1, B * (self._M + 1), self.d_k),
        )[0]
        if self.use_map_lanes:
        ...
        mode_probs = (
            self.prob_predictor(mode_params_emb).squeeze(-1).view(self.c, B, self._M + 1).sum(2).transpose(0, 1)
        )
