...
from ped_path_predictor.CI3PP.CVAE_CI3PP_Both.model import CVAE_CI3PP_Both
import time
import numpy as np
import torch
...
class CVAE_CI3PP_Both_Wrapper:
    ...
    def __init__(self, path=None):
        ...
        self.logger.info(f"Model: CVAE_CI3PP_Both")
        ...
        self.model = CVAE_CI3PP_Both(n_obs, n_pred).cuda()
    ...
    def train(self):
        ...
        for epoch in range(0, 1000):
            ...
            for i, (x, y) in enumerate(self.train_loader):
                ...
                pred_obs, mean, log_var = self.model(ped_in, cf_in, car_in, ego_out)
    ...
    def eval(self, dataloader):
        ...
        with torch.no_grad():
            for j, (x_val, y_val) in enumerate(dataloader):
                ...
                pred_obs = self.model.inference(ego_in, agents_in, map_lanes)
...
if __name__ == '__main__':
    ...
    if "--test" in sys.argv:
        abw_eval = CVAE_CI3PP_Both_Wrapper(path="./ped_path_predictor/saved_models/15_20/c_ci3pp_both.pth")
        ...
        print("\n\n\n")
        print("INT")
        print(f"&CI3P+(Cognitive+Car+AS+CVAE) & {int_1_a:.4f}/{int_1_f:.4f} & {int_2_a:.4f}/{int_2_f:.4f} & {int_3_a:.4f}/{int_3_f:.4f} & {int_4_a:.4f}/{int_4_f:.4f} & {int_5_a:.4f}/{int_5_f:.4f} & {int_6_a:.4f}/{int_6_f:.4f} \\\\")
        print("NON-INT")
        print(f"&CI3P+(Cognitive+Car+AS+CVAE) & {non_int_1_a:.4f}/{non_int_1_f:.4f} & {non_int_2_a:.4f}/{non_int_2_f:.4f} & {non_int_3_a:.4f}/{non_int_3_f:.4f} & {non_int_4_a:.4f}/{non_int_4_f:.4f} & {non_int_5_a:.4f}/{non_int_5_f:.4f} & {non_int_6_a:.4f}/{non_int_6_f:.4f} \\\\")
    else:
        abw = CVAE_CI3PP_Both_Wrapper()
        abw.train()