...
class AutoBotEgoCog(nn.Module):
    ...
    def __init__(self, d_k=128, _M=5, c=5, T=30, L_enc=1, dropout=0.0, k_attr=2, map_attr=3,
        ...
        super(AutoBotEgoCog, self).__init__()
        ...
        self.cf_gru = nn.GRU(input_size=d_k, hidden_size=d_k, batch_first=True)
        ...
        self.decoder_linear = nn.Sequential(
            init_(nn.Linear(d_k*2, 128)), nn.ReLU())
    ...
    def forward(self, ego_in, agents_in, roads, cognitive_in):
        ...
        cognitive_emb = self.cognitive_encoder(cognitive_in)
        cf_emb, _ = self.cf_gru(cognitive_emb)
        cf_emb = cf_emb.permute(1, 0, 2)

        car_soctemp_emb = agents_emb[:, :, 1]

        mh_traj_x_cf, _ = self.mha_traj_x_cf(ego_soctemp_emb, cf_emb, cf_emb)
        mh_cf_x_traj, _ = self.mha_cf_x_traj(cf_emb, ego_soctemp_emb, ego_soctemp_emb)

        stacked = torch.cat((mh_traj_x_cf, mh_cf_x_traj), dim=-1)

        ego_soctemp_emb = self.decoder_linear(stacked)
