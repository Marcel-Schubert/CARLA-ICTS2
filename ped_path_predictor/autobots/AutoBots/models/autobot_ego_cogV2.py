...
from ped_path_predictor.autobots.AutoBots.models.context_encoders import MapEncoderCNN, MapEncoderPts
...
class AutoBotEgoCogV2(nn.Module):
    ...
    def __init__(self, d_k=128, _M=5, c=5, T=30, L_enc=1, dropout=0.0, k_attr=2, map_attr=3,
        ...
        # ============================== AutoBot-Ego DECODER ==============================
        self.Q = nn.Parameter(torch.Tensor(self.T, 1, self.c, self.d_k*2), requires_grad=True)
        nn.init.xavier_uniform_(self.Q)

        self.tx_decoder = []
        for _ in range(self.L_dec):
            self.tx_decoder.append(nn.TransformerDecoderLayer(d_model=self.d_k*2, nhead=self.num_heads,
        ...
        self.P = nn.Parameter(torch.Tensor(c, 1, d_k*2), requires_grad=True)  # Appendix C.2.
        ...
        self.prob_decoder = nn.MultiheadAttention(self.d_k*2, num_heads=self.num_heads, dropout=self.dropout)
        self.prob_predictor = init_(nn.Linear(self.d_k*2, 1))


        # =========== CUSTOM CROSS ATTENTION ===============

        self.cognitive_encoder = nn.Sequential(init_(nn.Linear(2, d_k)))

        self.mha_traj_x_cf = nn.MultiheadAttention(embed_dim=d_k, num_heads=4, batch_first=False)
        self.mha_traj_x_car = nn.MultiheadAttention(embed_dim=d_k, num_heads=4, batch_first=False)

        self.mha_cf_x_traj = nn.MultiheadAttention(embed_dim=d_k, num_heads=4, batch_first=False)
        self.mha_cf_x_car = nn.MultiheadAttention(embed_dim=d_k, num_heads=4, batch_first=False)

        self.mha_car_x_traj = nn.MultiheadAttention(embed_dim=d_k, num_heads=4, batch_first=False)
        self.mha_car_x_cf = nn.MultiheadAttention(embed_dim=d_k, num_heads=4, batch_first=False)

        self.traj_gru = nn.GRU(input_size=d_k*2, hidden_size=d_k, batch_first=False)
        self.cf_gru = nn.GRU(input_size=d_k*2, hidden_size=d_k, batch_first=False)
        self.car_gru = nn.GRU(input_size=d_k*2, hidden_size=d_k, batch_first=False)

        self.decoder_linear = nn.Sequential(
            init_(nn.Linear(d_k*3, d_k)), nn.ReLU())
    ...
    def forward(self, ego_in, agents_in, roads, cognitive_in):
        '''
        ...
        ego_only = agents_emb[:, :, 0]
        car_only = agents_emb[:, :, 1]

        cognitive_emb = self.cognitive_encoder(cognitive_in).transpose(0, 1)


        mh_traj_x_cf, _ = self.mha_traj_x_cf(ego_only, cognitive_emb, cognitive_emb)
        mh_traj_x_car, _ = self.mha_traj_x_car(ego_only, car_only, car_only)

        mh_cf_x_traj, _ = self.mha_cf_x_traj(cognitive_emb, ego_only, ego_only)
        mh_cf_x_car, _ = self.mha_cf_x_car(cognitive_emb, car_only, car_only)

        mh_car_x_traj, _ = self.mha_car_x_traj(car_only, ego_only, ego_only)
        mh_car_x_cf, _ = self.mha_car_x_cf(car_only, cognitive_emb, cognitive_emb)

        stacked = torch.cat((mh_traj_x_cf, mh_cf_x_traj), dim=-1)
        ...
        traj_gru, _ = self.traj_gru(stacked_traj)
        cf_gru, _ = self.cf_gru(stacked_cf)
        car_gru, _ = self.car_gru(stacked_car)

        stacked = torch.cat([traj_gru, cf_gru, car_gru], dim=-1)

        decoded_lin = self.decoder_linear(stacked)
        ...
        ego_soctemp_emb = torch.concat((ego_soctemp_emb, decoded_lin), dim=-1) # add the cognitive encoding
        ...
        context = context.view(-1, B*self.c, self.d_k*2)
