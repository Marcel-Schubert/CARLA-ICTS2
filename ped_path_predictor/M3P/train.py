...
class M2P3Wrapper:
    ...
    def __init__(self, path=None):
        ...
        self.logger.info(f"Model: M2P3Wrapper")
        ...
        self.model = M2P3(n_obs, n_pred).cuda()
    ...
    def transform(self, x, y):
        ...
        ped_in = ped_in.transpose(0, 1)
        cf_in = cf_in.transpose(0, 1)
        car_in = car_in.transpose(0, 1)
        ped_out = ped_out.transpose(0, 1)


        return ped_in, cf_in, car_in, ped_out
    ...
    def train(self):
        ...
        for epoch in range(0, 1000):
            ...
            for i, (x, y) in enumerate(self.train_loader):
                ...
                ped_in, cf_in, car_in, ped_out = self.transform(x, y)

                output, mean, log_var = self.model((ped_in, ped_out))

                loss = self.l2_loss_fde(output.unsqueeze(0).transpose(1,2), ped_out)

                kld_loss = torch.mean(-0.5 * torch.sum(1 + log_var - mean ** 2 - log_var.exp(), dim=2), dim=1)
                ...
                if i % 100 == 0:
                    self.logger.info(
                        f"Epoch: {epoch:4}, Batch: {i:4} Loss: {loss.item():.6f} Time: {(time.time() - t_before): 4.4f}")
                    t_before = time.time()
    ...
    def eval(self, dataloader):
        ...
        with torch.no_grad():
            for j, (x_val, y_val) in enumerate(dataloader):
                ...
                ped_in, cf_in, car_in, ped_out = self.transform(x_val, y_val)

                pred_obs = self.model.inference(ped_in)

                ade_losses, fde_losses, a, f = self._compute_ego_errors(pred_obs, ped_out, ego_in=ped_in)
...
if __name__ == '__main__':
    if "--test" in sys.argv:
        abw_eval = M2P3Wrapper(path="./ped_path_predictor/saved_models/15_20/m3p.pth")
        ...
        print("INT")
        print(
            f"&M3P & {int_1_a:.4f}/{int_1_f:.4f} & {int_2_a:.4f}/{int_2_f:.4f} & {int_3_a:.4f}/{int_3_f:.4f} & {int_4_a:.4f}/{int_4_f:.4f} & {int_5_a:.4f}/{int_5_f:.4f} & {int_6_a:.4f}/{int_6_f:.4f} \\\\")
        print("NON-INT")
        print(
            f"&M3P & {non_int_1_a:.4f}/{non_int_1_f:.4f} & {non_int_2_a:.4f}/{non_int_2_f:.4f} & {non_int_3_a:.4f}/{non_int_3_f:.4f} & {non_int_4_a:.4f}/{non_int_4_f:.4f} & {non_int_5_a:.4f}/{non_int_5_f:.4f} & {non_int_6_a:.4f}/{non_int_6_f:.4f} \\\\")
