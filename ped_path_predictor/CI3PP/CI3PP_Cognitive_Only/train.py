...
from ped_path_predictor.CI3PP.CI3PP_Cognitive_Only.model import CI3PP_COG_Only
...
class CI3PP_COG_Only_Wrapper:
    ...
    def __init__(self, path=None):
        ...
        self.logger.info(f"Model: CI3PP_Cognitive_Only")
        ...
        self.model = CI3PP_COG_Only(n_obs, n_pred).cuda()
    ...
    def _compute_ego_errors(self, ego_preds, ego_gt, ego_in=None):
        with torch.no_grad():
            ...
            some = False
            if some == True:
                # index = ade_losses.argmax()
                # index = ade_losses.argmin()
                index = 0
                # # make output relative to the last observed frame
                i_t = ego_in[:, n_obs - 1:, 0:2].detach().cpu().numpy()
                ...
                plt.savefig("./pics/ci3p_cog_int5.svg", dpi=300)
                plt.savefig("./pics/ci3p_cog_int5.png", dpi=300)
                plt.show()
...
if __name__ == '__main__':
    ...
    if "--test" in sys.argv:
        abw_eval = CI3PP_COG_Only_Wrapper(path="./ped_path_predictor/saved_models/60_80/ci3pp_cognitive_only.pth")
        ...
        print("\n\n\n")
        print("INT")
        print(f"&CI3P+(Cognitive Only) & {int_1_a:.4f}/{int_1_f:.4f} & {int_2_a:.4f}/{int_2_f:.4f} & {int_3_a:.4f}/{int_3_f:.4f} & {int_4_a:.4f}/{int_4_f:.4f} & {int_5_a:.4f}/{int_5_f:.4f} & {int_6_a:.4f}/{int_6_f:.4f} \\\\")
        print("NON-INT")
        print(f"&CI3P+(Cognitive Only) & {non_int_1_a:.4f}/{non_int_1_f:.4f} & {non_int_2_a:.4f}/{non_int_2_f:.4f} & {non_int_3_a:.4f}/{non_int_3_f:.4f} & {non_int_4_a:.4f}/{non_int_4_f:.4f} & {non_int_5_a:.4f}/{non_int_5_f:.4f} & {non_int_6_a:.4f}/{non_int_6_f:.4f} \\\\")
    else:
        abw = CI3PP_COG_Only_Wrapper()
        abw.train()