...
class Decoder(nn.Module):
    ...
    def forward(
        self, past_feature, z, batch_size_curr, agent_num_perscene, past_traj, cur_location, sample_num, mode="train"
    ):
        ...
        # cur_location_repeat = cur_location.repeat_interleave(80, dim=1)
...
class GroupNet(nn.Module):
    ...
    def forward(self, data):
        ...
        future_traj = (
            data["future_traj"].view(batch_size * agent_num, self.args.future_length, 2).to(device).contiguous()
        )
        ...
        if self.args.learn_prior:
            ...
            if self.args.ztype == "gaussian":
                pz_distribution = Normal(
                    mu=torch.zeros(past_feature.shape[0], self.args.zdim).to(past_traj.device),
        ...
        total_loss = loss_pred + loss_kl + loss_recover + loss_diverse
