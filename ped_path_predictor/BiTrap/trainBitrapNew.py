...
class BiTrapNew:
    ...
    def __init__(self, path=None):
        ...
        self.logger.info(f"Model: BiTrapNew")
        self.logger.info(f"Observation Frames: {n_obs}")
        self.logger.info(f"Prediction Frames: {n_pred}")
        self.logger.info(f"Batch Size: {batch_size}")
        self.logger.info(f"Initial Learning Rate: {lr}")
        cfg.merge_from_file('./ped_path_predictor/BiTrap/bitrap_np_ICTS.yml')

        self.model = BiTraPNP(cfg.MODEL).cuda()
    ...
    def transform(self, x, y):
        ...
        # i_t = ped_in[:, 60 - 1:, 0:2].detach().cpu().numpy()
    ...
    def _compute_ego_errors(self, ego_preds, ego_gt, ego_in=None):
        with torch.no_grad():
            ...
            # ego_gt = torch.tensor(ego_gt).transpose(0, 1).unsqueeze(0).cuda()
            ...
            if some == True:
                ...
                ego_gt = ego_gt.squeeze()
                ego_gt = ego_gt[:, :, :2].cpu().numpy() + i_t

                ego_preds = ego_preds.squeeze()
                ego_preds = ego_preds[:, :, :2].cpu().numpy() + i_t

                plt.plot(ego_in[511, :, 0].cpu().numpy(), ego_in[511, :, 1].cpu().numpy())
                plt.plot(ego_gt[511, :, 0], ego_gt[511, :, 1])
                plt.plot(ego_preds[511, :, 0], ego_preds[511, :, 1])
                plt.show()
    ...
    def train(self):
        ...
        for epoch in range(0, 1000):
            ...
            for i, (x, y) in enumerate(self.train_loader):
                ...
                pred_goal, y_pred, loss_dict, _, _ = self.model(ped_in, target_y=ego_out)

                # ade_losses, fde_losses, a, f = self._compute_ego_errors(y_pred, ego_out, ego_in=ped_in)

                loss = loss_dict['loss_traj'] + \
                       loss_dict['loss_goal'] + \
                       kl_weight * loss_dict['loss_kld']
                self.model.param_scheduler.step()

                loss_dict = {k: v.item() for k, v in loss_dict.items()}
                loss_dict['lr'] = self.optimiser.param_groups[0]['lr']
    ...
    def eval(self, dataloader):
        ...
        with torch.no_grad():
            for j, (x_val, y_val) in enumerate(dataloader):
                ...
                ade_losses, fde_losses, a, f = self._compute_ego_errors(y_pred, ego_out, ego_in=ego_in)
...
if __name__ == '__main__':
    if "--test" in sys.argv:
        ...
        abw_eval = BiTrapNew(path='./ped_path_predictor/saved_models/15_20/biTrap.pth')
        ...
        print("\n\n\n")
        print("INT")
        print(f"&BiTrap & {int_1_a:.4f}/{int_1_f:.4f} & {int_2_a:.4f}/{int_2_f:.4f} & {int_3_a:.4f}/{int_3_f:.4f} & {int_4_a:.4f}/{int_4_f:.4f} & {int_5_a:.4f}/{int_5_f:.4f} & {int_6_a:.4f}/{int_6_f:.4f} \\\\")
        print("NON-INT")
        print(f"&BiTrap & {non_int_1_a:.4f}/{non_int_1_f:.4f} & {non_int_2_a:.4f}/{non_int_2_f:.4f} & {non_int_3_a:.4f}/{non_int_3_f:.4f} & {non_int_4_a:.4f}/{non_int_4_f:.4f} & {non_int_5_a:.4f}/{non_int_5_f:.4f} & {non_int_6_a:.4f}/{non_int_6_f:.4f} \\\\")
    else:
        abw = BiTrapNew()
        abw.train()