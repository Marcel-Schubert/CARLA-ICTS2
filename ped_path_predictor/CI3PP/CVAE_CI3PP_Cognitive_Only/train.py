...
from ped_path_predictor.CI3PP.CVAE_CI3PP_Cognitive_Only.model import CVAE_CI3PP_Cog_Only
import os
import time
import numpy as np
import torch
...
class CVAE_CI3PP_Cog_Only_Wrapper:
    ...
    def __init__(self, path=None):
        ...
        self.logger.info(f"Model: SocialCi3pWrapperV3CVAE_COG_ONLY")
        ...
        self.model = CVAE_CI3PP_Cog_Only(n_obs, n_pred).cuda()
    ...
    def train(self):
        ...
        for epoch in range(0, 1000):
            ...
            for i, (x, y) in enumerate(self.train_loader):
                ...
                pred_obs, mean, log_var = self.model(ped_in, cf_in, ego_out)

                loss = self.l2_loss_fde(pred_obs.unsqueeze(0).transpose(1, 2), ego_out)

                kld_loss = torch.mean(-0.5 * torch.sum(1 + log_var - mean ** 2 - log_var.exp(), dim=2), dim=0)
    ...
    def eval(self, dataloader):
        ...
        with torch.no_grad():
            for j, (x_val, y_val) in enumerate(dataloader):
                ...
                ped_in, cf_in, car_in, ego_out = self.transform(x_val, y_val)

                pred_obs = self.model.inference(ped_in, cf_in)

                ade_losses, fde_losses, a, f = self._compute_ego_errors(pred_obs.unsqueeze(0).transpose(1,2), ego_out, ego_in=ped_in)
...
if __name__ == '__main__':
    if "--test" in sys.argv:
        ...
        abw_eval = CVAE_CI3PP_Cog_Only_Wrapper(path="./_out/SocialCi3pWrapperV3CVAE_COG_ONLY/obs15_pred20/2024-07-15_00-34-58/model_174.pth")
        ...
        print("\n\n\n")
        print("INT")
        print(f"&CI3P+(Cognitive+AS+CVAE) & {int_1_a:.4f}/{int_1_f:.4f} & {int_2_a:.4f}/{int_2_f:.4f} & {int_3_a:.4f}/{int_3_f:.4f} & {int_4_a:.4f}/{int_4_f:.4f} & {int_5_a:.4f}/{int_5_f:.4f} & {int_6_a:.4f}/{int_6_f:.4f} \\\\")
        print("NON-INT")
        print(f"&CI3P+(Cognitive+AS+CVAE) & {non_int_1_a:.4f}/{non_int_1_f:.4f} & {non_int_2_a:.4f}/{non_int_2_f:.4f} & {non_int_3_a:.4f}/{non_int_3_f:.4f} & {non_int_4_a:.4f}/{non_int_4_f:.4f} & {non_int_5_a:.4f}/{non_int_5_f:.4f} & {non_int_6_a:.4f}/{non_int_6_f:.4f} \\\\")
    else:
        abw = CVAE_CI3PP_Cog_Only_Wrapper()
        abw.train()