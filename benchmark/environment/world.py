...
class World(object):
    ...
    def restart(self, scenario, conf=ControllerConfig()):
        ...
        if scenario_type == "01_int":
            ...
            cam_transform = carla.Transform(
                carla.Location(spawn_point.location.x, spawn_point.location.y - 25, spawn_point.location.z + 7),
            ...
            cam_transform = carla.Transform(
                carla.Location(spawn_point.location.x, spawn_point.location.y - 35, spawn_point.location.z + 7),
                carla.Rotation(-30, 270, 0),
            )
            self.world.get_spectator().set_transform(cam_transform)
            if not self.random:
            ...
            cam_transform = carla.Transform(
                carla.Location(spawn_point.location.x, spawn_point.location.y - 20, spawn_point.location.z + 7),
                carla.Rotation(-30, 270, 0),
            )
            self.world.get_spectator().set_transform(cam_transform)
            if not self.random:
        ...
        if scenario_type == "01_multi":
            self.choice = None
            self.setup_01_multi(obstacles, conf)
            cam_transform = carla.Transform(
                carla.Location(spawn_point.location.x, spawn_point.location.y - 25, spawn_point.location.z + 7),
                carla.Rotation(-30, 270, 0),
            )
            ...
            cam_transform = carla.Transform(
                carla.Location(spawn_point.location.x, spawn_point.location.y + -10, spawn_point.location.z + 5),
                carla.Rotation(-30, 270, 0),
            )
            self.world.get_spectator().set_transform(cam_transform)
            if not self.random:
            ...
            cam_transform = carla.Transform(
                carla.Location(spawn_point.location.x, spawn_point.location.y + 5, spawn_point.location.z + 15),
                carla.Rotation(-30, 270, 0),
            )
            self.world.get_spectator().set_transform(cam_transform)
            if not self.random:
            ...
            cam_transform = carla.Transform(
                carla.Location(spawn_point.location.x, spawn_point.location.y - 20, spawn_point.location.z + 15),
                carla.Rotation(-30, 270, 0),
            )
            self.world.get_spectator().set_transform(cam_transform)
            if not self.random:
    ...
    def tick(self, clock):
                ...
                if self.choice == "Stop":
                    ...
                    if l2_distance(self.walker.get_location(), self.flip_p) < 0.1 and self.flip_choice is None:
                        ...
                        if self.decision_trigger(
                            distance, self.slow_db, without_speed=True
                    ...
                    if l2_distance(self.walker.get_location(), self.acc_p) < 0.1 and self.flip_choice == "Error":
                        distance = y_distance(self.walker.get_location(), self.player.get_location()) - 2
                        if self.decision_trigger(
                            distance, self.acc_db, without_speed=True
                ...
                if status2 == "Done":
                    ...
                    if self.choice == "Back":
                        ...
                        self.walker.set_transform(
                            carla.Transform(self.walker.get_transform().location, carla.Rotation(0, 0, 0))
                        )
        ...
        if self.scenario[0] == "01_multi":
            status = self.path_controller_1.step()
            if self.dummy_car:
                self.car_controller.step()
            status_w2 = self.path_controller_w2_1.step()
            # self.look_behind_right.step()
    ...
    def get_walker2_state(self):
        loc = self.walker2.get_location()
        x, y = loc.x, loc.y
        return (x, y, self.walker2.icr, self.walker2.son)
    ...
    def setup_04_non_int(
        ...
        if self.dummy_car:
            self.car_controller = CarController(self.player, braking_point=None, speed=8, yielding=False)
    ...
    def setup_06_non_int(
        ...
        self.iss_crossed = InternalStateSetter(self.walker, path_1[0], icr=ICR.VERY_LOW, son=SON.AVERTING)
        if self.dummy_car:
            self.car_controller = CarController(self.player, braking_point=None, speed=5, yielding=False)
    ...
    def setup_03_non_int(
        ...
        if self.dummy_car:
            self.car_controller = CarController(self.player, braking_point=None, speed=4, yielding=False)
    ...
    def setup_02_non_int(self, obstacles, conf):
        ...
        if self.dummy_car:
            self.car_controller = CarController(self.player, braking_point=None, speed=3, yielding=False)
    ...
    def setup_03_int(self, obstacles, conf):
        ...
        self.path_2 = self._compute_plans(
            offsets_2, base_loc, color=carla.Color(r=255, g=0, b=0) if self.debug else None
        )
        ...
        self.turn_head = TurnHeadLeftWalk(
            self.walker, start_pos=self.get_p_from_vector(spawn_loc, path_1[0], looking_distance), char=conf.char
        )
        ...
        if self.dummy_car:
            self.car_controller = CarController(
                self.player, breaking_point, speed=speed if car_behave else speed, yielding=car_behave
            )
    ...
    def setup_02_int(self, obstacles, conf):
        ...
        self.path_2 = self._compute_plans(
            offsets_2, base_loc, color=carla.Color(r=0, g=255, b=0) if self.debug else None
        )
        self.path_controller_2 = PathController(self.world, self.walker, self.path_2, self.ped_speed)
        ...
        if self.dummy_car:
            self.car_controller = CarController(
                self.player, breaking_point, speed=speed if car_behave else speed - 1, yielding=car_behave
            )
    ...
    def setup_01_int(self, obstacles, conf):
        ...
        if self.debug:
            self.world.debug.draw_point(
                path_1[0] + 0.2 * vec, size=0.1, color=carla.Color(r=0, g=255, b=255), life_time=0
            )
            self._draw_grid()
            self._draw_db()
        self.relaxer = Relaxer(self.walker, self.player, path_1[0] + 0.2 * vec)
        ...
        if self.dummy_car:
            self.car_controller = CarController(
                self.player, breaking_point, speed=speed - 1 if car_behave else speed, yielding=car_behave
            )
    ...
    def setup_04_int(self, obstacles, conf):
        ...
        offsets_4 = [
            (crossing_distanceX - walk_back_distance - 0.2 * crossing_distanceX, walking_distance + crossing_distanceY)
        ]
        ...
        self.starts_crossing = InternalStateSetter(
            self.walker, path_1[-1], icr=ICR.GOING_TO, son=self.walker.initial_son
        )
        ...
        if self.dummy_car:
            self.car_controller = CarController(
                self.player, breaking_point, speed=speed - 2 if car_behave else speed - 1, yielding=car_behave
            )
    ...
    def setup_05_int(self, obstacles, conf):
        ...
        for uncert_p in range(uncertain_steps):
            points.append(self.get_point((walking_distance_X + (uncert_p + 1) * delta, -walking_distance_Y)))
        ...
        if self.dummy_car:
            self.car_controller = CarController(
                self.player, breaking_point, speed=speed - 1 if car_behave else speed - 0.5, yielding=car_behave
            )
    ...
    def setup_06_int(self, obstacles, conf):
        ...
        # Avoid the car
        offsets_3 = [(crossing_distance + car_avoid_X, -car_avoid_Y), (12, -car_avoid_Y), (12, -car_avoid_Y + 20)]
        self.path_3 = self._compute_plans(
            offsets_3, base_loc, color=carla.Color(r=0, g=0, b=255) if self.debug else None
        )
        ...
        self.raise_arm = RaiseArm(
            self.walker, path_1[0], "forcing", self.get_p_from_vector(path_1[0], self.path_2[0], 0.5)
        )
        self.look_behind_left = LookBehindLeft(self.walker, mult=2)
        self.reset = ResetPose(self.walker)
    ...
    def setup_01_multi(self, obstacles, conf):
        ...
        base_loc = obstacles[0][1].location + carla.Location(0, -spawning_distance, 0)
        spawn_loc = base_loc  # + carla.Location(-5,0,0)
        spawn_loc_w2 = base_loc + carla.Location(-1.5, 0, 0)
        self.walker = self.world.try_spawn_actor(obstacles[0][0], carla.Transform(spawn_loc, obstacles[0][1].rotation))
        self.walker.apply_control(carla.WalkerControl(carla.Vector3D(0, 0, 0), self.ped_speed))
        self.walker2 = self.world.try_spawn_actor(
            obstacles[1][0], carla.Transform(spawn_loc_w2, obstacles[1][1].rotation)
        )
        ...
        # Walk to the other side of the road
        offsets_2 = [(12, 11), (12, 20)]
        self.path_2 = self._compute_plans(
            offsets_2, base_loc, color=carla.Color(r=0, g=255, b=0) if self.debug else None
        )
        self.path_controller_2 = PathController(self.world, self.walker, self.path_2, self.ped_speed * mult)

        path_w2 = self._compute_plans(
            [(0, 100)], spawn_loc_w2, color=carla.Color(r=0, g=0, b=255) if self.debug else None
        )
        self.path_controller_w2_1 = PathController(self.world, self.walker2, path_w2, self.ped_speed)
    ...
    def _draw_circle(self, loc, radius):
        for i in range(0, 360, 2):
            ...
            self.world.debug.draw_point(
                loc + carla.Location(-x, y, 0), size=0.05, color=carla.Color(255, 165, 0), life_time=0
            )
    ...
    def compute_collision_point(self):
        ...
        self.world.debug.draw_line(
            walker_loc, walker_loc + 2 * walker_vel, thickness=0.05, color=carla.Color(255, 255, 255)
        )
